...
该案例展示了一个带约束的多目标背包问题的求解。
"""

# 添加几个容器来存储P、R、C
P = np.array([[3, 4, 9, 15, 2],
                    [4, 6, 8, 10, 2.5],
                    [5, 7, 10, 12, 3],
                    [3, 5, 10, 10, 2]])
R = np.array([[0.2, 0.3, 0.4, 0.6, 0.1],
                    [0.25, 0.35, 0.38, 0.45, 0.15],
                    [0.3, 0.37, 0.5, 0.5, 0.2],
                    [0.3, 0.32, 0.45, 0.6, 0.2]])
C = np.array([[10, 13, 24, 32, 4],
                    [12, 15, 22, 26, 5.2],
                    [14, 18, 25, 28, 6.8],
                    [14, 14, 28, 32, 6.8]])
...
def evalVars(Vars):  # 目标函数
    x = Vars.astype(int)
    f1 = np.sum(P[x, [0, 1, 2, 3, 4]], 1)
    f2 = np.sum(R[x, [0, 1, 2, 3, 4]], 1)
    # 采用可行性法则处理约束
    CV = np.array([np.sum(C[x, [0, 1, 2, 3, 4]], 1)]).T - 92
